...
(function(definition) {
	...
	function extend(classDefinition, superclass) {
		if (typeof classDefinition !== 'function') { throw new TypeError(msg(ERROR_MESSAGES.SUBCLASS_NOT_CONSTRUCTOR)); }
		...
		for (var key in classDefinition.prototype) {
		...
		for (var staticProperty in superclass) {
			if (superclass.hasOwnProperty(staticProperty)) {
				classDefinition[staticProperty] = superclass[staticProperty];
			}
		}
	...
	function mixin(target, mix) {
		...
		// Don't want any of the functions if there was an error.
	...
	function inherit(target, parent) {
		...
		if ( isAssignableFrom(target, parent) ) { return target; }

		var mixins = nonenum(target, '__multiparents__', []);
		var targetPrototype = target.prototype, mixinProperties = parent.prototype, resultingProperties = {};
		var attributeMap = attributesFromParents(parent);
		for (var key in mixinProperties) {
			var source = attributeMap[key];
			if ( key in targetPrototype === false ) {
				resultingProperties[key] = mixinProperties[key];
			} else if (source == null || isAssignableFrom(target, source) === false) {
				throw new Error(msg(ERROR_MESSAGES.ALREADY_PRESENT, key, className(parent, 'parent'), className(target, 'target')));
			}
		}
		// don't want any of the methods if there is an error.
		copy(resultingProperties, targetPrototype);
		mixins.push(parent);
		return target;
	}
	...
	function implement(classDefinition, protocol) {
		if (typeof classDefinition !== 'function') { throw new TypeError(msg(ERROR_MESSAGES.NOT_CONSTRUCTOR, 'Class', 'implement', typeof(classDefinition))); }
	...
	function fallbackIsAssignableFrom(classDefinition, parent) {
		...
		// parent
		var superPrototype = classDefinition.prototype.superclass || Object.getPrototypeOf(classDefinition.prototype);
	...
	function isAssignableFrom(classDefinition, constructor) {
		...
		if (typeof classDefinition !== 'function') { throw new TypeError(msg(ERROR_MESSAGES.NOT_CONSTRUCTOR, 'Class', 'isAssignableFrom', typeof(classDefinition))); }
	...
	function fulfills(instance, protocol) {
		if (instance == null) { throw new TypeError(msg(ERROR_MESSAGES.NULL, 'Object', 'fulfills')); }
		if (protocol == null) { throw new TypeError(msg(ERROR_MESSAGES.NULL, 'Protocol', 'fulfills')); }
	...
	function classFulfills(classDefinition, protocol) {
		if (classDefinition == null) { throw new TypeError(msg(ERROR_MESSAGES.NULL, 'Class', 'classFulfills')); }
		if (protocol == null) { throw new TypeError(msg(ERROR_MESSAGES.NULL, 'Protocol', 'classFulfills')); }
	 ...
	 * This function makes straight objects behave that way.
	 * @private
	 */
	...
	/**
	 * Returns a map of all the attributes specified by parents
	 * (mixins, parents from multiple inheritance, and superclasses), where the key
	 * is attribute name and the value is where they were specified.
	 * @private
	 */
	function attributesFromParents(classdef) {
		var result = {};

		var parentProto = Object.getPrototypeOf(classdef.prototype);
		if (parentProto != null && parentProto !== classdef.prototype) {
			...
			copy(attributesFromParents(parent), result);
		}

		// mixins and multiple inheritance chains.
		var mixins = nonenum(classdef, '__multiparents__', []);
		for (var i = 0; i < mixins.length; ++i) {
			copy(attributesFromParents(mixins[i]), result);
		}
