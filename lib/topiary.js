...
(function() {
	...
	/**
	 * Mixes functionality in to a class.  Only functions are mixed in.
	 * Code in the mixin is sandboxed and only has access to a 'mixin instance' rather than
	 * the real instance.
	 *
	 ...
	 * This is discouraged; you should prefer to use aggregation first,
	...
	function fallbackIsAssignableFrom(classdef, parent) {
		...
		var i, mixins = classdef.__multiparents__ || [], interfaces = classdef.__interfaces__ || [];

		// parent
		var superPrototype = Object.getPrototypeOf(classdef.prototype);
		if (superPrototype != null && isAssignableFrom(superPrototype.constructor, parent)) {
			return true;
		}
	...
	function fulfills(instance, protocol) {
		...
		// This is just a caching wrapper around fallbackFulfills
	...
	function classFulfills(classdef, protocol) {
		if (classdef == null) { throw new TypeError(msg(ERROR_MESSAGES.NULL, 'Class', 'classFulfills')); }
